...
async function loadGalleryData() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to load ${GALLERY_DATA_FILE}`);
    }

    const gallery = await response.json();

    renderGallery(galleryGrid, gallery);
    setupGalleryFilters();
...
function setupGalleryLightbox() {
  ...
  galleryItems = Array.from(cards).map(card => {
    ...
    const title =
      card.querySelector('.gallery-card__title');
  ...
  cards.forEach((card, index) => {
    const button =
      card.querySelector('.gallery-card__button');
